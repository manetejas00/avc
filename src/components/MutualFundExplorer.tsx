...
const MutualFundExplorer = () => {
  ...
  return (
    <section className="py-section-mobile md:py-section bg-surface-ii">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative mb-8">
            ...
            <input
              ...
              className="w-full bg-surface border border-border rounded-full py-4 pl-12 pr-4 text-text focus:outline-none focus:border-primary transition-colors"
            />
            {loading && (
          ...
          {results.length > 0 && !selectedFund && (
            <div className="bg-surface border border-border rounded-xl overflow-hidden mb-8 shadow-xl">
              <ul className="max-h-96 overflow-y-auto custom-scrollbar">
                {results.map(fund => (
                  <li 
                    key={fund.schemeCode}
                    onClick={() => fetchFundDetail(fund.schemeCode)}
                    className="border-b border-border last:border-0 p-4 hover:bg-surface-ii cursor-pointer transition-colors flex justify-between items-center"
                  >
          ...
          {selectedFund && (
            <div className="bg-surface border border-border rounded-2xl p-6 md:p-8 relative">
              <button 
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2 text-sm">
                  <span className="px-3 py-1 bg-surface-ii rounded-full text-text-muted">{selectedFund.meta.fund_house}</span>
                  <span className="px-3 py-1 bg-surface-ii rounded-full text-text-muted">{selectedFund.meta.scheme_category}</span>
                </div>
              </div>
